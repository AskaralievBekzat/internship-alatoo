# UniClub

## Description

UniClub is a platform that helps students discover university clubs, apply for membership, and track application statuses.

The platform also includes an AI-powered recommendation system that suggests suitable clubs based on students' interests.

---

## Features

- University email authentication
- Club directory
- Club information pages
- Membership application system
- Application status tracking
- Club president dashboard
- AI-powered club recommendations

---

## Tech Stack

### Frontend
- HTML
- CSS
- JavaScript

### Backend
- node.js

### AI
- Google AI Studio
- Groq

### DevOps
- Docker
- GitHub
- Railway


## Project Management

- Trello for task management and sprint tracking
- Daily progress reports
- Team coordination and meeting organization
- Documentation (BRD, Project Lifecycle, README)
- Project planning and task distribution

---

## Team Members

- Project Manager - Berdibekova Zhasmina EEAIR24
- Frontend Developer - Abdybekova Azema COMFCI24
- Backend Developer - Toktosunova Azema COMSE24
- DevOps Engineer - Askaraliev Bekzat COMSEH24
- AI Engineer - Nour Mahmoud Mohamed Mohamed Beshir EEAIR24

---

## Future Improvements

- AI chatbot for club-related questions
- Notifications and announcements
- Event calendar
- Additional university clubs
- Improved recommendation accuracy

---

TRELLO BOARD - https://trello.com/b/9xA0OdJp/моя-доска-trello
